      ...
      <div className="w-full max-w-3xl rounded-xl bg-[#fafafa] shadow-2xl flex flex-col max-h-[90vh]">
          ...
          {!hasItems ? (
            ...
            cartData.items.map((item) => (
              ...
              >
                ...
                {/* Participant Info */}
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm text-[#001819] bg-[#fafafa] p-3 rounded-md">
                  <div><span className="opacity-60">Name:</span> {item.participant.name}</div>
                  <div><span className="opacity-60">Gender:</span> {item.participant.gender}</div>
                  <div><span className="opacity-60">T-shirt:</span> {item.participant.tshirtSize}</div>
                  <div><span className="opacity-60">Blood:</span> {item.participant.bloodGroup}</div>
                  <div><span className="opacity-60">Contact:</span> {item.participant.contactNumber}</div>
                  <div><span className="opacity-60">District:</span> {item.participant.district}</div>
                </div>

                {/* Actions */}
                <div className="flex justify-end gap-3 border-t border-[#001819]/10 pt-3">
                  <button className="flex items-center gap-1 rounded-md border border-[#00a19a]/40 px-3 py-1.5 text-sm text-[#00a19a] hover:bg-[#00a19a]/10 transition-colors">
                    <Pencil size={14} /> Update
                  </button>
                  <button className="flex items-center gap-1 rounded-md border border-red-500/40 px-3 py-1.5 text-sm text-red-500 hover:bg-red-500/10 transition-colors">
        ...
        <div className="border-t border-[#001819]/10 bg-white px-6 py-4 shrink-0 rounded-b-xl">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button 
              disabled={!hasItems}
              className="rounded-lg bg-[#00a19a] px-8 py-2 text-sm font-semibold text-white hover:bg-[#009088] transition-all shadow-md active:scale-95 disabled:bg-gray-300 disabled:scale-100"
            >
              Proceed to Checkout
            </button>
          </div>
